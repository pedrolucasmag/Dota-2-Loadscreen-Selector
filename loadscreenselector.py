...
import platform
RESTORE_DEFAULT_HUD = "[Restore default LoadScreen.]"
...
def extract_skin(dota_directory, skin_name):
    data_path = 'root\\materials\\console\\loadingscreens\\' + skin_name
    out_path = dota_directory + "/materials/"
    ...
    try:
        rmtree(out_path + 'console/')  #otherwise, clear the default directory
        print('deleted %s ' % (out_path + 'console/'))
    except:
    ...
    if not os.path.exists(skin_path):
        ...
        print("Sucesso.")
    print("Setting new LoadScreen as default...")
    os.rename(skin_path, out_path + 'console')
    print("Renaming.....")
    prevname = dota_directory + '/materials/console/loadingscreen.vmt'
    newname  = dota_directory + '/materials/console/dashboard_loading_international_2012.vmt'
    os.rename(prevname, newname)
    prevname1 = dota_directory + '/materials/console/loadingscreen.vtf'
    newname1 = dota_directory + '/materials/console/dashboard_loading_international_2012.vtf'
    os.rename(prevname1, newname1)

def get_skin_name(filename):
    #I HATE REGEX I DON'T NEED IT
    prefix = 'root\\materials\\console\\loadingscreens\\'
    if not filename.startswith(prefix):  #if not vmt
        return None  #pular
...
def list_skins(dota_directory):
    ...
    if os.path.exists(dota_directory + "/materials/console/"):
        skins.append(RESTORE_DEFAULT_HUD)
    return skins


def uninstall(dota_directory):
    out_path = dota_directory + "/materials/console/"
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if not os.path.isdir(dota_directory):
            ...
            return
    print("777 Loading Screen Selector (wopaum@hotmail.com)")
    print("Searching for Loading Screens...")
    skins = list_skins(dota_directory)
    print("Available Loading Screens:")
    for idx, val in enumerate(skins):
        print("%s. %s" % (idx, val))
    user_input = ''
    while not (user_input.isdigit() and int(user_input) < len(skins)):
        user_input = input("Enter the number of the desired Loading Screen: ")
